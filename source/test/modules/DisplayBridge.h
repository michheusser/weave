...
namespace test
{
	namespace module
	{
		...
		public:
			...
			static Error::Result showFrame(const cv::Mat& frameBuffer, const std::string& title, const uint32_t frameID, const bool wait = false) noexcept;
			static void flushFrames();

		private:
			static Error::Result _showFrame(const cv::Mat& frameBuffer, const std::string& title, const uint32_t frameID, const bool wait = false) noexcept;
