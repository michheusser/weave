...
#include <weave/profiling/Hash.h>
		...
		#define TRACE_FUNCTION(className) weave::profiling::TraceSpan traceSpan##__LINE__ = weave::profiling::trace(std::string(className) + "::" + __FUNCTION__)
		...
		#define METRICS_COUNT(metricsName) \
			...
			weave::profiling::MetricsSpan metricsSpan##__LINE__ = weave::profiling::metric(metricsHash##__LINE__, metricsName, weave::profiling::constants::METRIC_COUNT_TYPE_STRING, 1)
		#define METRICS_BYTES(metricsName, data) \
			constexpr uint64_t metricsHash##__LINE__ = weave::profiling::Hash::hashString(metricsName);\
			weave::profiling::MetricsSpan metricsSpan##__LINE__ = weave::profiling::metric(metricsHash##__LINE__, metricsName, weave::profiling::constants::METRIC_BYTE_TYPE_STRING, data)
