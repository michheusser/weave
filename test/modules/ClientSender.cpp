...
namespace test
{
	namespace module
	{
		ClientSender::ClientSender(const Context<constants::ModuleType::ClientSender>& context) : _networkClient(context.networkClient), _encoder(context.encoderConfiguration),
		                                                                                          _bufferConfiguration(context.networkBufferConfiguration)
		{}

		void ClientSender::initialize()
		{
			_buffer.resize(_bufferConfiguration.bufferSize);
		}

		weave::error::Result ClientSender::process(const cv::Mat& inputBuffer) noexcept
		{
			...
			weave::error::Result resultSend = _networkClient->send(_buffer);
